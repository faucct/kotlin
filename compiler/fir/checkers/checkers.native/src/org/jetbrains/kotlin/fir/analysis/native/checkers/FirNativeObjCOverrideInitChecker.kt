...
object FirNativeObjCOverrideInitChecker : FirClassChecker() {
    override fun check(declaration: FirClass, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        fun FirConstructor.overridesConstructor(other: FirConstructorSymbol): Boolean {
            if (valueParameters.size != other.valueParameterSymbols.size)
                return false
            return valueParameters.zip(other.valueParameterSymbols).all { (thisParameter, otherParameter) ->
                thisParameter.name == otherParameter.name && thisParameter.returnTypeRef.coneType == otherParameter.resolvedReturnType
            }
        }
