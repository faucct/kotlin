...
internal fun Project.locateOrCreateCInteropDependencyConfiguration(
    ...
    return configurations.createResolvable(compilation.cInteropDependencyConfigurationName).apply {
        ...
        launchInStage(AfterFinaliseDsl) {
            ...
            attributes.setAttribute(project, LibraryElements.LIBRARY_ELEMENTS_ATTRIBUTE) { cinteropKlibLibraryElements() }
            attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { objects.named(Usage::class.java, KotlinUsages.KOTLIN_CINTEROP) }
...
internal fun Project.locateOrCreateCInteropApiElementsConfiguration(target: KotlinTarget): Configuration {
    ...
    return configurations.createConsumable(configurationName).apply {
        ...
        launchInStage(AfterFinaliseDsl) {
            ...
            attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { objects.named(Usage::class.java, KotlinUsages.KOTLIN_CINTEROP) }
            attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { project.categoryByName(Category.LIBRARY) }
            attributes.setAttribute(project, artifactTypeAttribute) { NativeArtifactFormat.KLIB }
