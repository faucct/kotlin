        ...
        private fun configureAttributes(
            ...
            if (kotlinTarget.platformType != KotlinPlatformType.common) {
                project.configurations.getByName(kotlinTarget.apiElementsConfigurationName).run {
                    attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { KotlinUsages.producerApiUsage(kotlinTarget) }
                ...
                project.configurations.getByName(kotlinTarget.runtimeElementsConfigurationName).run {
                    attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { KotlinUsages.producerRuntimeUsage(kotlinTarget) }
                    attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { project.categoryByName(Category.LIBRARY) }
                    usesPlatformOf(kotlinTarget)
                }
            }
