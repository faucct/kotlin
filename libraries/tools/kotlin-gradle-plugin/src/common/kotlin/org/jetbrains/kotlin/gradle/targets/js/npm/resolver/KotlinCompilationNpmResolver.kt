...
class KotlinCompilationNpmResolver(
    ...
    private fun createAggregatedConfiguration(): Configuration {
        ...
        all.attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { KotlinUsages.consumerRuntimeUsage(target) }
        all.attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { project.categoryByName(Category.LIBRARY) }
        all.attributes.setAttribute(project, publicPackageJsonAttribute) { PUBLIC_PACKAGE_JSON_ATTR_VALUE }
        all.isVisible = false
