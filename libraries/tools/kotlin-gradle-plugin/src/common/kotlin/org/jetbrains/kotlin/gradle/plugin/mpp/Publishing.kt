...
internal fun Configuration.configureSourcesPublicationAttributes(target: KotlinTarget) {
    ...
    attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { KotlinUsages.producerRuntimeUsage(target) }
    attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { project.attributeValueByName(Category.DOCUMENTATION) }
    attributes.setAttribute(project, DocsType.DOCS_TYPE_ATTRIBUTE) { project.attributeValueByName(DocsType.SOURCES) }
    ...
    attributes.setAttribute(project, Bundling.BUNDLING_ATTRIBUTE) { project.attributeValueByName(Bundling.EXTERNAL) }
