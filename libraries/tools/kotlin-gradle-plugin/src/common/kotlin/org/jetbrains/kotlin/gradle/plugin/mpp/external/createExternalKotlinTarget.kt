...
private fun ExternalKotlinTargetImpl.setupApiElements(configuration: Configuration) {
    ...
    configuration.attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { KotlinUsages.producerApiUsage(this) }
    configuration.attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { project.objects.named(Category.LIBRARY) }
}

private fun ExternalKotlinTargetImpl.setupRuntimeElements(configuration: Configuration) {
    configuration.usesPlatformOf(this)
    configuration.attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { KotlinUsages.producerRuntimeUsage(this) }
    configuration.attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { project.objects.named(Category.LIBRARY) }
}
