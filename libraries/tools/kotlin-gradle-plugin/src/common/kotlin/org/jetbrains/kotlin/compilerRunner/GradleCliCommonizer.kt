...
internal fun Project.maybeCreateCommonizerClasspathConfiguration(): Configuration {
    return configurations.findResolvable(KLIB_COMMONIZER_CLASSPATH_CONFIGURATION_NAME)
        ?: project.configurations.createResolvable(KLIB_COMMONIZER_CLASSPATH_CONFIGURATION_NAME)
            .run {
                attributes.setAttribute(project, Category.CATEGORY_ATTRIBUTE) { objects.named(Category.LIBRARY) }
                attributes.setAttribute(project, LibraryElements.LIBRARY_ELEMENTS_ATTRIBUTE) { objects.named(LibraryElements.JAR) }
                attributes.setAttribute(project, Usage.USAGE_ATTRIBUTE) { usageByName(Usage.JAVA_RUNTIME) }
