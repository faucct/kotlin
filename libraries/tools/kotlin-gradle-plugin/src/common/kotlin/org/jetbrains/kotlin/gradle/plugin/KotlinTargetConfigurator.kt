...
fun Configuration.usesPlatformOf(target: KotlinTarget): Configuration {
    attributes.setAttribute(target.project, KotlinPlatformType.attribute) { target.platformType }
    ...
    if (target is KotlinNativeTarget) {
        attributes.setAttribute(target.project, KotlinNativeTarget.konanTargetAttribute) { target.konanTarget.name }
    }
    return this
}
...
private fun Configuration.setJavaTargetEnvironmentAttributeIfSupported(project: Project, value: String) {
    if (GradleVersion.current() >= GradleVersion.version("7.0")) {
        attributes.setAttribute(project, TargetJvmEnvironment.TARGET_JVM_ENVIRONMENT_ATTRIBUTE) {
            project.objects.named<TargetJvmEnvironment>(value)
        }
    }
}
